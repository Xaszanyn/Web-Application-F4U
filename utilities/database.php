...
function create_order_request($menu_id, $province_id, $district_id, $days, $time, $promotion, $name, $phone, $email, $address, $height, $weight, $allergy, $disease, $occupation, $extra)
{
    ...
    $query = "INSERT INTO order_requests(menu_id, province_id, district_id, days, time, promotion, name, phone, email, address, height, weight, allergy, disease, occupation, extra) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $result = mysqli_prepare($connection, $query);
